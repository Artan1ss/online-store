...
import { RepairResults } from '@/types/repair';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const invalidOrderItems = orderItems.filter((item: { product: any }) => !item.product);
    if (invalidOrderItems.length > 0) {
      ...
      const deleteResult = await prisma.orderItem.deleteMany({
        where: {
          id: {
            in: invalidOrderItems.map((item: { id: string }) => item.id)
          }
        }
      });
    ...
    const emptyOrders = orders.filter((order: { items: any[] }) => order.items.length === 0);
    if (emptyOrders.length > 0) {
      ...
      const deleteResult = await prisma.order.deleteMany({
        where: {
          id: {
            in: emptyOrders.map((order: { id: string }) => order.id)
          }
        }
      });
